...
int main (int argc, char *argv[])
{
    ...
    const int n = 10;//atoi(argv[1]); 
    const int m = 2000;//atoi(argv[2]);
...
{   
    for (int i = 0; i < n; i++){
        ...
        printf ("кол-во семерок в строке %d:\t%d\n", i+1, count);
    }
}
    ...
    // std::chrono::steady_clock::time_point end_time = std::chrono::steady_clock::now();
    // std::chrono::duration<double> time_span = duration_cast<std::chrono::duration<double>>(end_time - start_time);
